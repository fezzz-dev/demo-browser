...
/**
 * Reads the API key from chrome.storage.sync, then sends `text` to the
 * OpenAI Chat Completions endpoint and returns the assistant's reply.
 *
 * @param {string} text - The selected text to summarize.
 * @returns {Promise<string>} The summary returned by the model.
 * @throws {Error} When no API key is configured, the network request fails,
...
async function fetchSummary(text) {
  ...
  if (!openaiApiKey) {
    throw new Error(
      "No API key configured. Please open the extension options and enter your OpenAI API key."
    );
  }
